...
**Known gaps, not yet fixed:** the Research Agent can skip planned calls (it skipped 2 of 15 in one run); this is now flagged and degrades the run, but not enforced. Strategy once needed a retry because a follow-up question listed an assumption id (`ASM-1`) where only `UNK-n` ids are allowed. With no evidence-cited post-promo price the pricing matrix can carry no post-promo prices at all, which weakens the promo-cliff comparison.
